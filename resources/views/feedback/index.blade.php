<x-app-layout>
    ...
    <div id="feedback-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-6xl">
            ...
            <div class="flex gap-3">
                <div class="size-60 bg-purple-300 rounded-lg flex items-center justify-center overflow-hidden">
                    @if ($feedback->image)
                        <img src="{{ asset('storage/' . $feedback->image) }}" class="size-60 object-cover rounded-lg"
                            alt="feedback Image">
                    @else
    ...
    <script>
        ...
        document.addfeedbackListener("DOMContentLoaded", function () {
        ...
        function showfeedbackDetails(id, name, category, venue, date, status) {
        ...
        // Delete feedback (AJAX Request)
        function deletefeedback(feedbackId) {
            Swal.fire({
                title: "Are you sure?",
                text: "This feedback will be marked as deleted.",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    fetch(`/feedbacks/${feedbackId}/delete`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                document.getElementById(`feedback-${feedbackId}`).remove(); // Remove feedback from UI
                                Swal.fire("Deleted!", "The feedback has been marked as deleted.", "success");
                            } else {
